...
const hide = (location) => {
  ...
  const seek = () => {
    //reference a variable outside the seek function but inside the hide() function, meaning it forms a closure because hide() will have already executed(closed) but seek can still access hide's variables, even though it closed.
...
let startGame = hide('under the table'); // place hide into startgame with argument: "under the table", effectively placing seek into startGame - now startGame waits a call, which will effectively call seek - seek has closure over the hideLocation variable, which itself long ago stored the location variable - which itself now contains the string of "under the table", when we input it into the function as we called it in this line(Line 13). Is this correct?
